  ...
  return (
    <nav className=" flex justify-between bg-white w-[89vw] left-[5.4vw] rounded-full fixed top-12 p-2.5 pl-11">
      <div className="flex items-center gap-15">
        ...
        <ul className="flex text-gray-700 gap-9">
      ...
      {!session && <div className="flex gap-2">
       <Link href="/login"><button className="bg-gray-100 hover:bg-gray-200 py-5 px-7 text-lg cursor-pointer text-black rounded-lg">log in</button></Link> 
       <Link href="/signup"><button className="bg-gray-900 hover:bg-gray-800 py-5 px-7 text-lg cursor-pointer text-white rounded-full">Sign up free</button></Link> 
      </div>}
      {session && <><button onClick={() => {setShowdropdown(!showdropdown)}} onBlur={()=>{setTimeout(() => {setShowdropdown(false)},200)}} id="dropdownDefaultButton" data-dropdown-toggle="dropdown" className="flex justify-center items-center text-white bg-gradient-to-br cursor-pointer from-purple-600 to-blue-900 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium text-lg disabled:from-purple-500 disabled:to-blue-500  me-2 text-center py-5 px-10  rounded-full" type="button">{(session.user.name).split("gmail")[0]}<svg className="w-2.5 h-2.5 ms-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 10 6">
